...
function RotatingFileStream(options) {
    ...
    // Options for enabling logging if file exists
    var runtimeEnablerFile;
    if (options.runtimeEnablerFile && typeof options.runtimeEnablerFile === 'string') {
        runtimeEnablerFile = options.runtimeEnablerFile;
    } else {
        runtimeEnablerFile = false;
    }
    var runtimeEnablerCheckInterval = options.runtimeEnablerCheckInterval || 15000;
    ...
    var lastCheckTime = 0;
    function checkIfWriteEnabled(cb) {

        if (!runtimeEnablerFile) {
            // enabled if no file provided
            cb(true);
        } else {
            // only check the file once per interval
            if (Date.now() - lastCheckTime >= runtimeEnablerCheckInterval) {
                lastCheckTime = Date.now();
                return fs.access(runtimeEnablerFile, function(err) {
                    if (err) {
                        writeEnabled = false;
                    } else {
                        writeEnabled = true;
            ...
            // return cached value most of the time
            } else {
                cb(writeEnabled);
            }
        }
    ...
    function write(s, callback) {
        checkIfWriteEnabled(function (isEnabled) {
            if (isEnabled) {
